...
def load_json(filename: str) -> Any | None:
    ...
    Cached for 2 s so tab switches don't replay 13 disk reads, while still
    refreshing fast enough that a live audit re-run is reflected on the next click.
...
with st.sidebar:
    ...
    audit_clicked = st.button("⚡ INITIATE AUTONOMOUS AUDIT", width="stretch")
  ...
  try:
    ...
    with col_recon:
        ...
        if headers_list:
            ...
            st.dataframe(df_h[keep], width="stretch", hide_index=True)
        else:
    ...
    if isinstance(boardroom, dict) or isinstance(exfil, dict):
        ...
        with gate_col1:
            st.button("✅  AUTHORIZE EXFILTRATION", width="stretch", disabled=True)
        with gate_col2:
            st.button("🛑  HALT & QUARANTINE", width="stretch", disabled=True)
